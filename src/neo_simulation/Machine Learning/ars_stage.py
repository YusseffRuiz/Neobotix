...
import numpy as np
import gym
from gym import wrappers  ## let us see our AI's walking on the field
...
class Normalizer():
    ...
    def observe(self, x):  ##x will be a new state
        ...
        self.mean_diff += (x - last_mean) * (x - self.mean)  ##the sum of the previous numerator + value of the state - the last mean times value of the state - new mean
        self.var = (self.mean_diff / self.n).clip(min=1e-2)  ##this never has to be zero and that why we use .clip(min=1e-2)!! Remember this tip.
...
class PolicyAI():
    def __init__(self, input_size, output_size):  ##return several actions
        self.theta = np.zeros((output_size, input_size))  ##vector of the different weights for the perceptron (rows, columns)

    ### evaluate function refers to step 5 of the proposed algorithm
    def evaluate(self, input, delta=None, direction=None):  ##delta is the perturbation added to define the different directions, It will be defined as None because it could not have perturbations
    ...
    def sample_deltas(self):  ##returning small values following a gaussian distribution
        ...
        ##*self.theta.shape gives the exact dimensions of the theta vector, only appliable in python3 // Already updated for Python 2
...
class ReinforceAgent():
    def __init__(self):
        ...
        self.episode_length = 1000  ##maximum length of an episode (time the AI will walk on the field)
    ...
    def train(self, env, policy, normalizer):
        ...
        for step in range(self.nb_steps):
            ...
            pos_r = [0] * self.nb_directions
            neg_r = [0] * self.nb_directions
            ...
            for k in range(self.nb_directions):
                pos_r[k] = self.explore(env, normalizer, policy, direction="positive", delta=delta[k])

            for k in range(self.nb_directions):
                neg_r[k] = self.explore(env, normalizer, policy, direction="negative", delta=delta[k])
            ...
            ##we want the keys sorted, so we put it as 1st value sorted by the maximum values
            ...
            reward_evaluation = self.explore(env, normalizer, policy)
...
if __name__ == '__main__':
    ...
    hp = ReinforceAgent()  ### we are creating an Hp class object and all variables are already initialized
    np.random.seed(hp.seed)

    ## Prepare out policy
    nb_inputs = state_size
    nb_outputs = action_size
    ...
    hp.train(env, policy, normalizer)